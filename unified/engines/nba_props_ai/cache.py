...
class SQLiteCache:
    def __init__(self, path: str = "nba_props_cache.sqlite"):
        self.path = str(path)
    ...
    def _hash_key(self, endpoint: str, params: dict) -> str:
        blob = json.dumps({"endpoint": endpoint, "params": params}, sort_keys=True, default=str).encode("utf-8")
    ...
    def get(self, endpoint: str, params: dict) -> Optional[CacheHit]:
        ...
        con = sqlite3.connect(self.path)
        cur = con.cursor()
        cur.execute("SELECT asof_utc, data_json FROM snapshots WHERE key = ?", (key,))
        row = cur.fetchone()
        con.close()
        if not row:
            return None
    ...
    def set(self, endpoint: str, params: dict, data: Any, asof_utc: Optional[str] = None):
        key = self._hash_key(endpoint, params)
        if asof_utc is None:
        ...
        cur = con.cursor()
        cur.execute(
            "REPLACE INTO snapshots (key, endpoint, params_json, asof_utc, data_json) VALUES (?,?,?,?,?)",
            (key, endpoint, json.dumps(params, sort_keys=True, default=str), asof_utc, json.dumps(data, default=str)),
        )
        con.commit()
        con.close()
        ...
        *,
        ...
        cur.execute(
            ...
            INSERT INTO endpoint_health (
              endpoint, calls, success_count, failure_count, last_success_utc, last_failure_utc, last_error, updated_utc
            )
            VALUES (
              ?, 1, ?, ?, ?, ?, ?, ?
            )
    ...
    def get_endpoint_health(self, endpoint: str) -> Optional[EndpointHealth]:
        ...
        cur.execute(
            """
            SELECT endpoint, calls, success_count, failure_count, last_success_utc, last_failure_utc, last_error
        ...
        rate = float(success / calls) if calls > 0 else 0.0
